...
theorem sum_conjClasses_card_eq_card [Fintype <| ConjClasses G] [Fintype G]
  ...
  suffices h_equiv : (Σ x : ConjClasses G, x.carrier) ≃ G by simpa using (Fintype.card_congr h_equiv)
...
theorem Group.nat_card_center_add_sum_card_noncenter_eq_card [Finite G] :
  ...
  classical
  cases nonempty_fintype G
  rewrite [@Nat.card_eq_fintype_card G, ← sum_conjClasses_card_eq_card, ←
